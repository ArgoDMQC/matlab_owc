...
%use qc flags if required; this is default unless fl.useqc passed into function
    if strcmp(fl.useqc, '0')
        ...
        for row = 1:size(sqc, 1)
            ...
            for flag = 1:length(fl.useqc) 
                ...
                prownan(pkeep) = 0;
            end % for flag = 1:length(fl.useqc) 
            S(row, :) = S(row, :) + srownan; %discard unwanted data by adding 0 (keep) or nan (discard)
            T(row, :) = T(row, :) + trownan;
            P(row, :) = P(row, :) + prownan;
        end % for row = 1:size(sqc, 1)
    end % if strcmp(fl.useqc, '0')
    ...
    for k = prof_range(1):prof_range(2)
        ...
        else % if(length(igood) < 3)
            sint = interp1(w2(igood, 1), w2(igood, 2), theta_base(:));
            Sint(:, k) = sint;
        end % if(length(igood) < 3)
    end % for k = prof_range(1):prof_range(2)
...
%breck--
        ...
        else% if length(y2) > 1 & length([x1:x2]) > 1
            plot(nan, nan);
        end % if length(y2) > 1 & length([x1:x2]) > 1
    else % if ~isnan(smin)
