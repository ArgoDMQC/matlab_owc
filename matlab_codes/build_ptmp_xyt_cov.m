...
function cov = build_ptmp_xyt_cov( ptmp,coord_float,po_system_configuration);
...
% the small time scales are also taken into account when building the covariance between different mapped profiles.
...
scale_t = str2num(po_system_configuration.MAPSCALE_AGE_SMALL);
map_use_pv = str2num(po_system_configuration.MAP_USE_PV);


for k=1:n
  b(k,:)=covarxyt_pv(coord_float(k,:),coord_float,scale_lon,scale_lat,scale_t,scale_phi,map_use_pv);
end
b=b(:,1:n);
